...
public class Test02 {
	public Test02() {
		// 기본생성자
		// 생성자 함수는 반환값이 없다. 반환값 타입을 기술하면 안된다.
//		abc();
		
		// 함수는 자기 자신이 스스로 실행되는 경우는 없다.
		// 랜덤한 두 정수 만들고
		int no1 = makeInt();
		int no2 = makeInt();
	...
	public int makeInt() {	// 속성은 필요할 경우에만 기술한다.
		// 반환값 변수
		int no = 0;
...
//		return;	// return 명령을 기술하지 않아도 실행내용을 모두 실행했으므로
				// 이 함수가 자동 종료될 수 있다.
				// 따라서 생략해도 상관없다.
	}
	...
	public String toPrint(int no1, int no2, int no3) {
		// 참고 ] 
		// 		매개변수 리스트에 정의된 변수는 이 함수 블럭에서만 사용할 수 있는
		//		지역변수이다.
		...
		/*
			함수의 오버로딩
				...
				원칙 ]
					...
					1. 함수이름이 같아야한다.
					2. 매개변수 리스트가 달라야한다.
						(매개변수 리스트의 타입, 순서, 갯수 중 하나 이상 달라야한다.)	
					3. 반환값타입과는 무관하다. (void)			
						
						참고 ]
							void abc(int a, int b){}
							void abc(int b, int a){}	==> X
							매개변수리스트의 형태는 데이터의 타입으로 판별한다.
