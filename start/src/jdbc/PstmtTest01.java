...
/*
	...
	따라서 이 클래스는 객체가 되는 순간
	데이터베이스에 연결할 준비가 되어있어야 한다.
		...
		try {
			...
			/*
				불완전한 질의명령(? 있는 질의명령)에 데이터를 채우는 방법)
					...
					이 때 위치값은 문자열 중에서 ? 의 순서를 의미하고
					시작값은 1부터 시작한다.
			 */
			pstmt.setInt(1, dno);
			
			// 질의명령이 완성이 됬으므로 JDBC 에게 전달하고 결과를 받는다.
			rs = pstmt.executeQuery();
			// 몇 사람이 조회될지 알 수 없으므로
			while(rs.next()) {
				// rs.next() ==> 작업실행줄을 한 줄 내려주는 함수.
				//				 이때 내린 결과가 꺼내올 데이터가 있으면
				//				 true 를 반환해준다. 없으면 false.
				...
				// 출력
				System.out.printf("%5d - %15s - %10s - %5d - %10s - %5d\n", eno, name, job, mgr, hdate, dno);
			}
