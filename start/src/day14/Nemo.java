...
public class Nemo {
	...
	public Nemo() {
		...
		// 만약 생성자를 하나라도 만들면 JVM 이 기본생성자를 만들지 않는다.
		// 따라서 생성자를 정의할 경우는 반드시 내용이 비어있는
		// 기본생성자도 정의하는것이 좋다.
	}
	...
	// 면적 계산해서 세팅해주는 함수
	public void setArea() {
		// 이 함수를 호출하는 순간은 이미 가로와 세로 길이가 결정된 후 일 것이다.
		// 따라서 이미 필요한 데이터가 준비되어있고 접근이 가능하므로
		// 따로 데이터를 전달받지 않아도 된다.
		area = width * height;
	}
	public int getWidth() {
