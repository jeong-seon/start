...
public class Test05 {
	...
	public Test05() {
		...
		while(token.hasMoreTokens()) {
			// token.hasMoreTokens()
			// ==> 임시메모리에 꺼낼 문자열이 있는지 여부를 논리값으로 반환
			// token.hasMoreElements()
			// ==> 임시메모리에 꺼낼 객체가 있는지 여부를 논리값으로 반환
		...
		System.out.println("==================================");
		// 꺼낸 데이터를 다시 꺼낼수는 없다. (사용하는 순간 메모리에서 사라짐)
		while(token.hasMoreTokens()) {
			
			// token.nextElement() 함수의 반환값은 Object 타입으로 반환해준다.
//			String black = (String)(token.nextElement());
			String black = token.nextToken();
