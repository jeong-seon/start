 ...
 */
	...
	public Jjokji() {
		...
		// 목록상자에 친구 이름을 기록해서 넣어보자.
		// Map 에서 친구 이름만 찾아서 넣어보자.
		...
		/*
			...
			네트워크 구성을 할 차례이다.
			여기서는 UDP 통신을 해야하므로
			DatagramSocket 을 이용해서 네트워크를 구성해야한다.
		 */
		try {
			...
			// 쪽지를 보내고 받을 수 있다.
			// 받는 프로그램을 시작한다.
			// 받는 프로그램은 다른 프로그램들과 병행해서 처리가 되어야 하므로
			// 스레드로 구성한다.
			...
			// 위 작업이 예외가 발생한 것은
			// 이 프로그램이 더이상 제 기능을 할 수 없는 상태이므로
			// 열려있는 소켓을 모두 닫아준다.
			close();
			e.printStackTrace();
		}
