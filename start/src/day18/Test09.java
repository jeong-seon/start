 ...
 */
	public Test09() {
		...
		// 이것을 실행하기 위해서는 스레드 프로그램이 Runnable 상태로 전위되면된다.
		
		// New Born 상태의 스레드 프로그램을 Runnable 상태로 전위 시키는 명령은
		// start() 명령이다.
		sohyun.start();
		seora.start();
...
class MyThread01 extends Thread {
	// 이 클래스 내에서는 변수나 다른 함수들을 만들 수 있지만
	// 여기서는 테스트만 해보기로 한다.
	@Override
	public void run() {
		// 이 프로그램에서 코딩을 1000개를 하도록 하게 할 것이다.
