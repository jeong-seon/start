...
public class EmpView {
	...
	public int getEno(Scanner sc) {
		...
		while(true){
			enoPrint();
			System.out.print("사원번호를 입력하세요! 이전단계는 -1 을 입력하세요.\n사원번호 : ");
	...
	public void enoPrint() {
		ArrayList<EmpVO> list = eDao.getEmpnoList();
		
		// 출력
		for(EmpVO evo : list) {
			System.out.println(evo.getEno() + " - " + evo.getEname());
		}
		System.out.println();
	}
	...
	public void enoInfoPrint(Scanner sc) {
		while(true) {
			...
			ArrayList<EmpVO> list = eDao.getEnoInfo(eno);
			
			if(list.size() == 0) {
				System.out.println("*** 사원번호가 없는 사원입니다.\n");
				continue;
			}
			
			System.out.println("*** " + eno + "번 사원 정보조회");
			System.out.println("===========================================================================");
